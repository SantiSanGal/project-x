...
export const login = async ({ request, response, auth }: HttpContextContract) => {
    ...
    try {
        ...
        const tempPasswordEntry = await Database
        ...
        if (tempPasswordEntry && await Hash.verify(tempPasswordEntry.temporary_password, password)) {
            ...
            params.notification.message = 'Inicio Sesión Correctamente con Contraseña Temporal'
